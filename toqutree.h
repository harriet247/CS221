...
private:
  int sizeHelper(Node * croot);
  PNG iterateAsistPNG(PNG* fill,unsigned int width,unsigned int height);
  void iterateNewPNG(const PNG *fill,PNG*& dist,unsigned int width,int xLoc,int yLoc);
  pair<int,int> getCenter(stats *stats,unsigned int width);
  void copyColor(HSLAPixel * pixel, const HSLAPixel * other);
  PNG renderHelper(const Node * croot);
  PNG renderLoopHelper(const PNG& fill, PNG* dist,int xLoc,int yLoc,unsigned int width);
  void pruneHelper(Node*& croot,double tol);
  bool determinePrune(Node* croot,Node*child,double tol);
  void destroyAll(Node*& croot);
  Node *copyHelper(const Node* croot);
   /*
